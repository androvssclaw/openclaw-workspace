...
- [x] Добавить `setup_v5_cron.sh` для установки/проверки всех ключевых cron job'ов одним запуском.
- [x] Добавить верификацию расписаний + idempotent update (не плодить дубли).

## 2) Task quality guardrails
- [x] Добавить `task.sh lint` (проверка дубликатов, битых `due:`, невалидных `ctx:`).
